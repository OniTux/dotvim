...
set foldenable
" Affichage d'une linge de '=' sur les pliages
set fillchars=fold:=
" Définition personnelle de l'affichage du pliage
set foldtext=MyFoldFunction()
" Variable indiquant le nom de la session en cours d'utilisations
let s:sessionName = ''

"Détection d'une session en cours d'utilisation
function! s:IsSessionInUse()
    if (s:sessionName != '')
        call s:DisplayStatus('La session "' . s:sessionName . '"' . " est en cours d'utilisation. Veuillez quitter Vim pour charger une nouvelle session.")
...
function! s:GetSessionName()
    ...
    if (!match(l:tmpSessionName, '^[a-zA-Z*\n$'))
        call s:DisplayStatus('Abandon : un nom de session ne doit contenir que des lettres')
...
function! SaveSession()
    ...
    if (s:sessionName =='')
        ...
        if (s:sessionDesc == '')
            let s:sessionDesc = 'Pas de desciption'
        endif
        if filereadable($HOME . '/.vim/sessions/' . s:sessionName . '.vim')
            let l:saveSession = confirm('Une session du même nom existe déjà! Écraser ?' ,"&Oui\n&Non", 1)
...
function! LoadTemplate(extension, type)
    ...
    " Inserion des commandes spécifiques Vim
...
" Fonction d'affichag/masquage de l'aide
function! HelpMemory()
    if g:helpDisplay == 0
...
" Evite de recharcher les fichiers distants lorsque l'on navique les
" buffers
set bufhidden=hide
